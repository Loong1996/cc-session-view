...
export function App() {
  ...
  useEffect(() => {
    async function loadSessions() {
      ...
      setError(null)
      try {
        const [claude, codex] = await Promise.all([listClaudeCodeSessions(), listCodexSessions()])
  ...
  async function handleExport(format: "text" | "html" | "markdown") {
    ...
    const dateStr = selectedSession.timestamp.toISOString().slice(0, 10)
    const filename = `${selectedSession.agentType}--${dateStr}--${selectedSession.id}.${ext}`
  ...
  async function handleViewInBrowser() {
    ...
    const dateStr = selectedSession.timestamp.toISOString().slice(0, 10)
    const filename = `${selectedSession.agentType}--${dateStr}--${selectedSession.id}.html`
        ...
        <>
          ...
          <Box marginTop={1} flexDirection="row">
            <Box
              ...
              marginRight={layout.showPreview ? 1 : 0}
            >
              ...
              <SessionList
                ...
                width={layout.listWidth}
                highlightIndex={
                  activeTab === "claude-code" ? claudeHighlightIndex : codexHighlightIndex
                }
                onHighlightIndexChange={
                  activeTab === "claude-code" ? setClaudeHighlightIndex : setCodexHighlightIndex
                }
              />
            </Box>
